...
#Files
```{r}
interview <- readxl::read_excel("~/Marlena - Sickle Cell/Data/scInterviewData.WedJun191007002019.xlsx")
...
```{r}
#Sex
#df$Sex <- ifelse(df$Sex == "F", 1, 0) #1= female
df$Sex <- as.factor(df$Sex)

#Race
#df$Race <- ifelse(df$Race == "W", 1, 0) #1= white
df$Race <- as.factor(df$Race)

#Income
#df$Income <- ifelse(df$Income == "Prefer not to answer" | df$Income == "Don't know", NA, df$Income)

#df$Income <- ifelse(df$Income == "$20,000 or less" | df$Income == "$20,001 to $35,000" | df$Income == "$35,001 to $50,000", 1, 0) #1 = less than 50k
df$Income <- as.factor(df$Income)

#Educ
#df$Educ <- ifelse(df$Educ == "Some High School" | df$Educ == "High School Graduate" | df$Educ == "Associate Degree/ Some College" | df$Educ == "General Education Development (GED)", 1, 0) #1 = less than Bachelors
df$Educ <- as.factor(df$Educ)

#Marstat
#df$MarStat <- ifelse(df$MarStat == "Married/ Living as Married", 1, 0) #1= married
